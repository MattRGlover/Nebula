...
const TARGET_FRAME_TIME = 33.33; // 30fps target = 33.33ms per frame
const LAG_THRESHOLD = 50; // Frame taking >50ms is a lag spike
const DROP_THRESHOLD = 100; // Frame taking >100ms likely dropped frames
...
const WATER_CELL_SIZE = 6; // Grid cell size for water simulation (mobile: 6 for high definition)
...
function setup() {
  ...
  // Force pixel density to 1 for mobile performance (prevents retina scaling)
  pixelDensity(1);
  ...
  // Initialize water ripple simulation at very low resolution for mobile performance
  ...
  // Cap frame rate for mobile battery/performance
  frameRate(30);
...
function generateWatercolorBackground(pg, time = 0, dt = 16.666) {
  ...
  const numSplotches = 8; // Match desktop config
  const arr_num = 150; // Balanced shape count
  const shapeScale = 0.55; // Smaller individual shapes
  ...
  for (let i = 0; i < numSplotches; i++) {
    ...
    for (let k = 0; k < arr_num; k++) {
      ...
      let form = transformShape(points, 2, 0.5, pg); // Reduced recursion for mobile
      arr.push(form);
    }
...
function addWaterDisturbance(x, y, vx = 0, vy = 0, isDrag = false) {
  ...
  maxRippleRadius = min(waterCols, waterRows) * 0.08; // Smaller spread on mobile (8% of grid, was 12%)
...
function drawDebugGrid() {
  ...
  textAlign(LEFT, TOP);
  textSize(14);
  const fpsColor = currentFPS >= 25 ? color(0, 255, 0) : (currentFPS >= 15 ? color(255, 255, 0) : color(255, 0, 0));
  fill(fpsColor);
